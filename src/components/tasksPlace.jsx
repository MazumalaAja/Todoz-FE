...
// ===== Tasks Place =====
export default function TasksPlace({ label, date }) {
     // ===== Status =====
     ...
     // ===== Peogress Icons =====
     const Progress = [RiProgress1Line, RiProgress2Line, RiProgress3Line, RiProgress4Line, RiProgress5Line, RiProgress6Line, RiProgress7Line, RiProgress8Line]

     // ===== Progress Selected =====
     const [selected, setSelected] = useState(0)
     const isSuccess = selected === 7;

     const ProgressSelectedIcon = Progress[selected]
     return (
          <>
               {/* ===== Container ===== */}
               <div className={`flex ${checked.success || isSuccess ? `opacity-50 relative bg-green-400/20 text-green-100` : `bg-neutral-500/10 text-neutral-200`} items-center outline-1 outline-neutral-500/10 hover:outline-amber-500 duration-150 justify-between p-3 mb-2  cursor-pointer rounded-md`}>
                    ...
                    <div className="flex items-center gap-3">
                         ...
                         {/* ===== Information4s ===== */}
                         <div>
                              <h1 className={`${checked.success || isSuccess ? `line-through` : ``}`}>{label}</h1>
                              <small className="text-neutral-400 text-sm">{date}</small>
                         </div>
                    </div>
                    ...
                    <div onClick={() => setChecked(prev => ({ ...prev, progressSelected: !prev.progressSelected }))} className="text-neutral-300 flex items-center gap-3 duration-200">
                         {!checked.success ?
                              <ProgressSelectedIcon className={`text-3xl`} />
                              :
                              <RiProgress8Line className={`text-3xl`} />
                         }
